...
import { OpFile, Thing, ThingID, ThingValue } from '.';
import { DataWriter } from './data-writer';
import { FieldNotFound } from './exceptions/FieldNotFound';

export class ThingEditor {

    private fields: { [key: string]: { [key: string]: ThingValue[] } } = {}
    private relations: { [key: string]: ThingID[] } = {}

    constructor(public updater: DataWriter, private id?: ThingID, public langs: string[] = []) { }
    ...
    set(field_name: string, value: ThingValue, lang?: string, append = false): ThingEditor {
        return this.setValues(field_name, [value], lang, append)
    }

    //TODO type @values
    setValues(field_name: string, values: ThingValue[], lang?: string, append = false): ThingEditor {
    ...
    async copyFromThing(item: Thing, limit_langs: string[]): Promise<ThingEditor> {
        this.updater.resource.fields.filter(f => f.type != 'relation').forEach(field => {
            const remote_field = item.resource().field(field.name)
            if (!remote_field) return
            const langs = field.is_translatable ? limit_langs ?? this.langs ?? (this.updater.schema()).langs : [undefined]
            langs.forEach(lang => {
